...
def calc_point2point(predict, actual):
    ...
    try:
        ...
        roc_auc = 0
    return f1, precision, recall, accuracy, TP, TN, FP, FN, roc_auc
...
def bf_without_point_adjust_search(score, label, start, end=None, step_num=1, display_freq=10, verbose=True):
    """
    Find the best-f1 score by searching best `threshold` in [`start`, `end`).
    Method from OmniAnomaly (https://github.com/NetManAIOps/OmniAnomaly)
    """
...
def find_epsilon(errors, reg_level=1):
    ...
    for z in np.arange(1.5, 12, 0.25):
...
def blind_bf_search(
        score, label, val, start, end=None, step_num=1, guess=None, display_freq=1, verbose=True, tw=15, normal=0,
        direction='>'
):
    """
